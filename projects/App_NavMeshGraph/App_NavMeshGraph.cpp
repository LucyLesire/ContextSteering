...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	auto startTriangle = m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(startPos);
	auto endTriangle = m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(endPos);
	...
	NavGraphNode* startNode(new NavGraphNode(cloneGraph->GetNextFreeNodeIndex(), invalid_node_index, startPos)); //May need to use nextfreenode
	cloneGraph->AddNode(startNode);

	for (int lineIdx : startTriangle->metaData.IndexLines)
	{
		int graphNodeIdx = m_pNavGraph->GetNodeIdxFromLineIdx(lineIdx);
		if (graphNodeIdx != invalid_node_index)
		{
			cloneGraph->AddConnection(new GraphConnection2D(startNode->GetIndex(), graphNodeIdx, Distance(startPos, cloneGraph->GetNodePos(graphNodeIdx))));
		}
	}
	
	//Create extra node for the End Node
	NavGraphNode* endNode(new NavGraphNode(cloneGraph->GetNextFreeNodeIndex(), invalid_node_index, endPos)); //May need to use nextfreenode
	...
	for (int lineIdx : endTriangle->metaData.IndexLines)
	{
		...
		if (graphNodeIdx != invalid_node_index)
		{
			cloneGraph->AddConnection(new GraphConnection2D(endNode->GetIndex(), graphNodeIdx, Distance(endPos, cloneGraph->GetNodePos(graphNodeIdx))));
		}
	}
	//Run A star on new graph
	Elite::Heuristic heuristicFunction = Elite::HeuristicFunctions::Chebyshev;
	auto pathfinder = AStar<NavGraphNode, GraphConnection2D>(cloneGraph.get(), heuristicFunction);

	auto unOptimizedPath = pathfinder.FindPath(startNode, endNode);
	//Extra: Run optimiser on new graph, Make sure the A star path is fine before uncommenting this!
	m_Portals = SSFA::FindPortals(unOptimizedPath, m_pNavGraph->GetNavMeshPolygon());
