...
void Elite::NavGraph::CreateNavigationGraph()
{
	...
	for (auto line : m_pNavMeshPolygon->GetLines())
	{
		...
		{
			AddNode(new NavGraphNode(GetNextFreeNodeIndex(), line->index, Elite::Vector2{ (line->p1 + line->p2) / 2 }));
		}
	}
	//2. Create connections now that every node is created
	for (auto triangle : m_pNavMeshPolygon->GetTriangles())
	{
		std::vector<int> nodeLineIdxs;
		for (int lineIdx : triangle->metaData.IndexLines)
		{
			for (auto node : m_Nodes)
			{
				...
				{
					nodeLineIdxs.push_back(node->GetIndex());
				}
			}
		}
		if (nodeLineIdxs.size() == 2)
		{
			AddConnection(new GraphConnection2D(nodeLineIdxs[0], nodeLineIdxs[1]));
		}
		else if(nodeLineIdxs.size() == 3)
		{
			AddConnection(new GraphConnection2D(nodeLineIdxs[0], nodeLineIdxs[1]));
			AddConnection(new GraphConnection2D(nodeLineIdxs[1], nodeLineIdxs[2]));
			AddConnection(new GraphConnection2D(nodeLineIdxs[2], nodeLineIdxs[0]));
		}
	}
